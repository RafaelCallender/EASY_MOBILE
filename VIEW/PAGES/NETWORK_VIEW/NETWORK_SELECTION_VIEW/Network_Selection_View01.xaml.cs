...
using EASYPROJECT01.EASY_CALLENDER01.SERVICES.NETWORK_SERVICES;
using EASYPROJECT01.EASY_CALLENDER01.TEMPLATE.MOBILE_TEMPLATE.PICKER.PICKER_DEFAULT;

namespace EASYPROJECT01.EASY_MOBILE.VIEW.PAGES.NETWORK_VIEW.NETWORK_SELECTION_VIEW
{
    public partial class Network_Selection_View01 : ContentPage
    {
        ...
        private static string menu_string01 = $"scan network\n" +
                         $"network monitor\n" +
                         $"network_monitor01\n";
        ...
        private void load_Selection_data01()
        {
            for (int i = 0; i < menu_array01.Length; i++)
            {
                ...
                switch (menu_array01[picker01.SelectedIndex])
                {
                    ...
                    case "scan network":
                        _commands.Add(procedure, scan_network);
                        break;
                    case "network monitor":
                        _commands.Add(procedure, network_monitor);
                        break;
                    case "network monitor01":
                        _commands.Add(procedure, network_monitor01);
                        break;
        ...
        {
            label02.Text = Network_S01.scan_netwrok();
        }

        private void network_monitor()
        {
            label02.Text = Network_S01.network_monitor();
        }

        private void network_monitor01()
        {
            label02.Text = Network_S01.network_monitor01();
        }
        ...
        private async void button02_clicked(object? sender, EventArgs e)
        {
            ...
            await button02.Navigation.PushAsync(new Network_Main_View01());
        }
    }
}